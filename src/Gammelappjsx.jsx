...
async function restoreSession(sessionId) {
  ...
  function addPlayerToOngoingSession(playerId) {
  // 1. Legg til i checkedIn
  setCheckedIn(prev => [...prev, playerId]);

  // 2. Sett spiller bakerst i køen
  setWaitingQueue(prev => [...prev, playerId]);

  // 3. Toast
  showToast(`${playerName(playerId)} lagt til i økta ✓`, "success");
}
``
  ...
  const notInSessionPlayers = players.filter(p => !checkedIn.includes(p.id));
...
const sessionList = allSessions
  ...
  const shell = (children) => (
    <div style={{ minHeight: "100dvh", background: "#020617", color: "#f8fafc", fontFamily: "'Barlow', sans-serif", paddingBottom: 40, width: "100%", maxWidth: "480px", margin: "0 auto", position: "relative" }}>
  ...
  ➕ Legg til spillere
        ...
        {showAddPlayers && (
  <Card style  </Card>  <Card style={{ padding: "16px 18px", marginBottom: 16 }}>
)}
